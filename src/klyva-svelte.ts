...
export const slicedStore = <T>(
  store: Writable<Array<T>>,
): Readable<Array<Writable<T> & { remove: () => void }>> => {
  return {
    subscribe: (listen) => {
...
const sliceStore = <T>(
  store: Writable<Array<T>>,
): Array<Writable<T> & { remove: () => void }> => {
  const value: T[] = get(store)

  const sliceIsRemoved = (arr: Array<T>, index: number) => index >= arr.length
